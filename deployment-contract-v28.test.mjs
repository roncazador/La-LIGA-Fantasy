...
assert.equal(checks.length,29);for(const[i,[name,ok]]of checks.entries())assert.ok(ok,`DEPLOY28-${String(i+1).padStart(3,'0')}: ${name}`);console.log('DEPLOYMENT CONTRACT v5.8: 29/29 checks passed');
