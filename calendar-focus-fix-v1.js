...
function latestNextFixture(raw,team){const list=rows(raw,'matches').concat(rows(raw,'fixtures'),rows(raw,'merged')).map(x=>({home:first(x.home,x.homeTeam?.name,x.localTeam?.name),away:first(x.away,x.awayTeam?.name,x.visitorTeam?.name),date:first(x.utcDate,x.date,x.startDate,x.starting_at),matchday:first(x.matchday,x.officialMatchday,x.week,x.gameweek?.week)})).filter(x=>x.home&&x.away&&x.date&&Number.isFinite(Date.parse(x.date))&&[teamFrom(x.home),teamFrom(x.away)].includes(teamFrom(team))&&Date.parse(x.date)>=Date.now()).sort((a,b)=>Date.parse(a.date)-Date.parse(b.date));return list[0]||null}
