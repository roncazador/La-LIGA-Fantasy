...
check(css.includes('.nav button.active'), 'main navigation active state matches DOM');
for(const tab of ['inicio','brain','plantilla','xi','partidos','mercado','aciertos','radar','rivales','fuentes','datos','estado']) check(css.includes(`data-tab="${tab}"]::before`), `icon mapping exists for ${tab}`);
for(const tab of ['inicio','brain','plantilla','xi','partidos','mercado','aciertos','radar','rivales','fuentes','datos','estado']) check(html.includes(`data-tab="${tab}"`), `DOM exposes ${tab} navigation tab`);
check(css.includes('scroll-snap-type:x proximity'), 'mobile tab strip is thumb friendly');
check(css.includes('overflow-x:hidden'), 'page prevents horizontal bleed');
check(css.includes('prefers-reduced-motion:reduce'), 'reduced-motion safeguard exists');
