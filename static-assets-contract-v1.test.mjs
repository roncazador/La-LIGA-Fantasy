...
for (const asset of ['/app-dynamics-v37.js','/calendar-autonomous-v35.js','/match-detail-ui-v31.js','/focus-ui-v30.js','/futbolfantasy-ui-v30.js']) assert.ok(config.includes(`'${asset}'`), `STATIC-${asset} must be publicly served`);
for (const pattern of ["loadInline('/app-dynamics-v37.js','37')","loadInline('/calendar-autonomous-v35.js','35')","loadInline('/focus-ui-v30.js','30')","loadInline('/futbolfantasy-ui-v30.js','30')","loadInline('/match-detail-ui-v31.js','33')"]) assert.ok(connection.includes(pattern), `loader must use inline-safe path: ${pattern}`);
const loadedAssets=[...connection.matchAll(/['"](\/[^'"]+\.(?:js|json))['"]/g)].map(m=>m[1]);
const missing=loadedAssets.filter(asset=>asset.startsWith('/')&&!config.includes(`'${asset}'`));assert.deepEqual(missing,[],`loader assets missing from publicStaticPath: ${missing.join(', ')}`);
