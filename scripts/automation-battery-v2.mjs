...
for(const file of walk(ROOT).filter(p=>/\.(js|mjs|json|yml|yaml|html)$/.test(p)).filter(p=>!/^automation-(battery|handoff)-report\./.test(path.basename(p)))){let text='';try{text=read(file)}catch{continue}push(`secret scan: ${file}`,!secret.test(text),'security')}
...
push('handoff has stable schema',exists('scripts/automation-handoff-v1.mjs')&&read('scripts/automation-handoff-v1.mjs').includes('laliga-automation-handoff/v1'),'ci-automation');
...
if(failed.some(r=>r.domain==='security'))rec.push('Revisar patrones de secretos detectados y confirmar que ningún credential llegue al cliente.');
