...
const needed=[];
function inspect(rule){const file=path.resolve(ROOT,rule.file);const before=fs.readFileSync(file,'utf8');if(before.includes(rule.replace))return false;if(before.includes(rule.find)){needed.push({file:rule.file,reason:rule.reason});if(mode==='--write')fs.writeFileSync(file,before.replace(rule.find,rule.replace),'utf8');return true}return false}
for(const rule of RULES)inspect(rule);
if(mode==='--check'&&needed.length){for(const item of needed)console.error(`AUTO-CORRECT-NEEDED ${item.file}: ${item.reason}`);process.exitCode=1}else if(mode==='--write')console.log(JSON.stringify({version:'laliga-auto-correct/v1',changed:needed,allowedFiles:[...allowed],count:needed.length}));else console.log('AUTO-CORRECT v1: clean; no deterministic correction required');
