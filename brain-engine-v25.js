(() => {
  ...
  function normalize(p) {
    return {
      ...
      price: num(p?.price ?? p?.marketPrice ?? p?.currentPrice),
  ...
  async function load(){
    try{
      ...
      render(Array.isArray(local)?local:[]);
      const body=document.getElementById('brain25Body');
      if(body && !local.length) body.insertAdjacentHTML('afterbegin',`<div class="source">Motor v2.5 en modo local: ${esc(error.message)}. No se inventan datos.</div>`);
    }
  }

  function boot(){ensureStyle();panel();load();}
