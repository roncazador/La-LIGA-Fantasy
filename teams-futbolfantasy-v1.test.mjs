...
for(const pattern of ["'/teams-futbolfantasy-v1.js'","loadInline('/teams-futbolfantasy-v1.js','1')"]){assert.ok(config.includes(pattern)||connection.includes(pattern)||host.includes(pattern),`asset wiring missing: ${pattern}`)}
...
assert.ok(ui.includes('source: FutbolFantasy'));
