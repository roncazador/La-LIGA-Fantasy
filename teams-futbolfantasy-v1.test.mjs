...
const realdata=fs.readFileSync('./realdata.mjs','utf8');
...
assert.ok(ui.includes('20 equipos · jugadores · clasificación · lesiones · calendario · solo lectura'));
assert.ok(ui.includes('Los valores no disponibles aparecen como'));
assert.ok(ui.includes('prefetchPlayers'));
assert.ok(ui.includes('Cargando jugadores'));
assert.ok(ui.includes('API_FOOTBALL_API_KEY'));
assert.ok(realdata.includes('STATIC_LALIGA_TEAMS'));
assert.ok(realdata.includes("source:'static-identity-fallback'"));
assert.ok(realdata.includes('normalized.length ? normalized : staticTeamFallback()'));
console.log('DATA CENTER UI v3: 20 teams · compact sections · automatic player prefetch · team fallback contract');
