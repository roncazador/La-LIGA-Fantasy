...
const INSTALL_KEY = Symbol.for('laliga.brain.historyHook.v28');
...
if (!globalThis[INSTALL_KEY]) {
      ...
      if (profile.found && profile.labeledSamples >= 2) {
        ...
        if (Number.isFinite(recent)) {
          const historicalSignal = Math.max(
            0,
            Math.min(1, recent / 20 + Math.max(-0.15, Math.min(0.15, trend / 20)))
          );
          const blend = 0.22;
          const score = Math.max(0, Math.min(1, prediction.score * (1 - blend) + historicalSignal * blend));
          return {
            ...prediction,
            score,
            expectedPoints: Math.max(0, Math.round((score * 20 + (prediction.features.media ?? 0) * 0.7) * 10) / 10),
            history: { samples: profile.labeledSamples, averagePoints: profile.averagePoints, recentAveragePoints: profile.recentAveragePoints, formTrend: profile.formTrend, blend }
          };
        }
      }
    } catch (error) {
  ...
  BrainV27.prototype.status = function statusWithHistory() {
    return { ...originalStatus.call(this), history: history.summary(), historyVersion: HISTORY_VERSION, reasoning: 'history-aware' };
  };
