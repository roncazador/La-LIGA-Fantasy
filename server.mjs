...
function getSession(req){
  ...
  if (Date.now() - session.createdAt > ttl) { sessions.delete(id); return null; }
  return session;
}
...
function serveStatic(res, pathname){
  ...
  if (file === 'index.html' && content.includes('</body>')) {
    const scripts = [
...
function oidcStatus(){
  ...
  return { configured: oidcConfigured(config), missing: required.filter(key => !process.env[key]), hasAuthorizeUrl: Boolean(config.laligaAuthorizeUrl), hasClientId: Boolean(config.laligaOAuthClientId), hasRedirectUri: Boolean(config.laligaRedirectUri), policy: config.laligaSigninPolicy };
}
...
async function refresh(session){
  ...
  const body = new URLSearchParams({ grant_type: 'refresh_token', refresh_token: session.refreshToken, client_id: config.laligaOAuthClientId, scope: 'openid offline_access' });
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/api/health' && req.method === 'GET') {
      return sendJson(res, 200, { ok: true, readOnly: true, competition: config.laligaCompetitionId, version: VERSION, providers: { laligaOAuth: oidcConfigured(config), footballData: Boolean(config.footballDataToken), ...providerStatus(config) } });
    }
    ...
    if (url.pathname === '/api/fixtures/next' && req.method === 'GET') {
      const data = await fetchMultiProviderFixtures(config);
      return sendJson(res, data.merged.length || data.primaryProvider ? 200 : 503, {
        ok: data.merged.length > 0 || Boolean(data.primaryProvider), readOnly: true,
        ...
        from: data.from, to: data.to, primaryProvider: data.primaryProvider,
        providers: Object.fromEntries(Object.entries(data.providers).map(([name, result]) => [name, { ok: result.ok, count: result.ok ? result.matches.length : 0, error: result.ok ? null : result.error }])),
        matches: data.merged
      });
    }
    if (url.pathname === '/api/providers/fixtures/next' && req.method === 'GET') return sendJson(res, 200, { ok: true, readOnly: true, ...(await fetchMultiProviderFixtures(config)) });
    if (url.pathname === '/api/providers/api-football/fixtures' && req.method === 'GET') {
      try {
        ...
        return sendJson(res, 200, { ok: true, provider: 'api-football', from, to, matches: await fetchApiFootballFixtures(config, from, to) });
      } catch (error) { return sendJson(res, error.status || 502, { error: error.message || 'API_FOOTBALL_FAILED' }); }
    }
    if (url.pathname === '/api/providers/sportmonks/fixtures' && req.method === 'GET') {
      try {
        ...
        const from = url.searchParams.get('from') || now.toISOString().slice(0, 10);
        const to = url.searchParams.get('to') || new Date(now.getTime() + config.footballDataDays * 86400000).toISOString().slice(0, 10);
        return sendJson(res, 200, { ok: true, provider: 'sportmonks', from, to, matches: await fetchSportmonksFixtures(config, from, to) });
      } catch (error) { return sendJson(res, error.status || 502, { error: error.message || 'SPORTMONKS_FAILED' }); }
    }
    if (url.pathname === '/api/providers/opta/status' && req.method === 'GET') return sendJson(res, 200, { provider: 'opta', ...providerStatus(config).opta });

    /* REAL DATA: API-FOOTBALL, read-only, lazy per section. */
    if (url.pathname === '/api/data/teams' && req.method === 'GET') return sendJson(res, 200, { ok: true, readOnly: true, provider: 'API-Football', competition: config.apiFootballLeagueId, season: config.apiFootballSeason, teams: await fetchTeams(config) });
    if (url.pathname === '/api/data/standings' && req.method === 'GET') return sendJson(res, 200, { ok: true, readOnly: true, provider: 'API-Football', competition: config.apiFootballLeagueId, season: config.apiFootballSeason, standings: await fetchStandings(config) });
    if (url.pathname === '/api/data/players' && req.method === 'GET') {
      const page = Number(url.searchParams.get('page') || 1);
      const data = await fetchPlayers(config, page);
      return sendJson(res, 200, { ok: true, readOnly: true, provider: 'API-Football', competition: config.apiFootballLeagueId, season: config.apiFootballSeason, ...data });
    }
    if (url.pathname === '/api/data/injuries' && req.method === 'GET') return sendJson(res, 200, { ok: true, readOnly: true, provider: 'API-Football', competition: config.apiFootballLeagueId, season: config.apiFootballSeason, injuries: await fetchInjuries(config) });
    ...
    if (url.pathname === '/auth/start' && req.method === 'GET') {
      ...
      res.writeHead(302, { Location: `${config.laligaAuthorizeUrl}?${query.toString()}`, 'Set-Cookie': cookieHeader(sessionId, 900) }); return res.end();
    }
    ...
    if (url.pathname === '/auth/callback' && req.method === 'GET') {
      ...
      if (!tokenResponse.ok || (!token.access_token && !token.id_token)) return sendJson(res, 502, { error: 'OIDC_TOKEN_EXCHANGE_FAILED', status: tokenResponse.status });
      sessions.set(sessionId, { createdAt: Date.now(), accessToken: token.access_token || token.id_token, refreshToken: token.refresh_token, expiresAt: Date.now() + Number(token.expires_in || 86400) * 1000 });
      const location = pending.platform === 'ios' ? config.iosSuccessRedirect : config.frontendUrl;
      res.writeHead(302, { Location: location, 'Set-Cookie': cookieHeader(sessionId, 2592000) }); return res.end();
    }
    if (url.pathname === '/auth/logout') {
      const sessionId = parseCookies(req)[config.sessionCookieName]; if (sessionId) sessions.delete(sessionId);
      res.writeHead(302, { Location: config.frontendUrl, 'Set-Cookie': cookieHeader('', 0) }); return res.end();
    }

    if (url.pathname === '/api/fantasy/dashboard') {
      if (req.method !== 'GET') return sendJson(res, 405, { error: 'GET_ONLY' });
      const session = getSession(req); if (!session) return sendJson(res, 401, { error: 'AUTH_REQUIRED' });
      ...
      if (leagueId) {
        const competition = encodeURIComponent(config.laligaCompetitionId); const encodedLeague = encodeURIComponent(leagueId);
        ...
        const rows = arrayData(output.standing); const profile = objectData(output.profile) || {}; const user = profile.username || profile.email || profile.name;
    ...
    if (url.pathname.startsWith('/api/fantasy/')) {
      ...
      const endpoint = fantasyPath(key, url); if (!endpoint) return sendJson(res, 400, { error: 'MISSING_PARAMETER' });
