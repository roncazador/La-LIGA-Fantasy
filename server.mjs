...
const VERSION='2.1.0';
const IOS_SUCCESS_REDIRECT=process.env.IOS_SUCCESS_REDIRECT||'laligafantasy://auth-complete';
...
function reply(res,status,body){res.writeHead(status,{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store','Access-Control-Allow-Origin':ORIGIN,'Access-Control-Allow-Credentials':'true','X-Content-Type-Options':'nosniff','Referrer-Policy':'no-referrer','X-Frame-Options':'DENY'});res.end(JSON.stringify(body));}
...
http.createServer(async(req,res)=>{
 ...
 if(u.pathname==='/api/health')return reply(res,200,{ok:true,readOnly:true,competition:COMP,version:VERSION});
 ...
 if(u.pathname==='/auth/start'){
  ...
  if(!authorize||!clientId||!redirectUri)return reply(res,501,{error:'OIDC_NOT_CONFIGURED',message:'Faltan parÃ¡metros OIDC oficiales.'});
  ...
  sessions.set(sid,{createdAt:Date.now(),state,verifier,platform:u.searchParams.get('platform')==='ios'?'ios':'web'});
