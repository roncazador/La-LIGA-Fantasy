...
for(const token of ["'/api/futbolfantasy/data'","'/api/data/standings'","'/api/fixtures'","'/api/data/players'",'Titular','Suplentes candidatos','media Fantasy son estimaciones','Estado de jugadores','Posible XI','Bajas / dudas','refreshStandings']) assert.ok(ui.includes(token),`missing ${token}`);
...
assert.ok(ui.includes('Fuente principal: FutbolFantasy público contrastado'),'missing source disclaimer');
